	...
	if (pIWMPMedia != NULL)
	{
		...
		if (m_lastPlayedSourceURL.Compare(sourceURL) != 0)
		{
			...
			if(extension.Find(L".m4a") > -1)
			{
				...
				HRESULT hr = Util::ReadMP4CoverData(fileName, notification.m_icon.m_data);
				if (SUCCEEDED(hr))
				{ 
					hr = Util::MD5HashData(notification.m_icon.m_data, notification.m_icon.m_dataHash);
			...
			else
			{
				...
				if (pMedia3 != NULL)
				{
					...
					hr = pMedia3->getItemInfoByType(L"WM/Picture", NULL, 0, &itemInfoVariant);
					if(SUCCEEDED(hr))
					{
						// QI the IWMPMetadataPicture from the IDispatch value of the variant
						CComQIPtr<IWMPMetadataPicture> pictureMetadata = itemInfoVariant.pdispVal;
						if (pictureMetadata != NULL)
						{
							...
							if (SUCCEEDED(hr))
							{
								...
								BOOL status = GetUrlCacheEntryInfo(metaURLBSTR, NULL, &cbCacheEntryInfo);
								
								// Allocate some memory on the heap for it and retrieve the entry
								cacheEntryInfo = (INTERNET_CACHE_ENTRY_INFO *) new BYTE[cbCacheEntryInfo];
								if (cacheEntryInfo)
								{
									status = GetUrlCacheEntryInfo(metaURLBSTR, cacheEntryInfo, &cbCacheEntryInfo);
									if (status)
									{
										CString actualPath = cacheEntryInfo->lpszLocalFileName;

										// Add the path onto the notification
										notification.m_icon.InitWithFilePath(actualPath);
									}
			...
			if (growlMessage.GetLength())
			{
				...
				if (m_growlClientRegistered)
				{
					notification.m_type = GrowlNotificationType(L"Item Changed");
...
void CWMPGNTP::PlaylistChange( IDispatch * Playlist, WMPPlaylistChangeEventType change )
{
    switch (change)
    {
    case wmplcUnknown:
...
void CWMPGNTP::MediaCollectionAttributeStringAdded( BSTR bstrAttribName,  BSTR bstrAttribVal )
{
}

void CWMPGNTP::MediaCollectionAttributeStringRemoved( BSTR bstrAttribName,  BSTR bstrAttribVal )
{
}

void CWMPGNTP::MediaCollectionAttributeStringChanged( BSTR bstrAttribName, BSTR bstrOldAttribVal, BSTR bstrNewAttribVal)
{
}
...
void CWMPGNTP::PlaylistCollectionPlaylistSetAsDeleted( BSTR bstrPlaylistName, VARIANT_BOOL varfIsDeleted)
{
}
